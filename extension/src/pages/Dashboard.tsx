...
function Dashboard() {
    ...
    <div className="w-full h-full flex flex-col items-center justify-center space-y-5">
      ...
      <div className="w-full flex flex-col flex-grow items-center justify-center space-y-2">
        ...
        <div className="w-full flex flex-col items-center justify-center space-y-2">
          {["Devices", "Access"].map((title) => (
            <div
              key={title}
              className="w-3/4 flex justify-between items-center flex-wrap p-4 rounded-xl shadow bg-white transition-colors hover:bg-gray-100 border border-gray-200 text-base text-gray-800 font-semibold"
            >
              <div className="w-full flex flex-wrap items-center gap-5">
                <h4 className="w-4/5 truncate text-gray-800 font-semibold">
                  {title}
                </h4>
          ...
          <Button className="w-3/4 bg-transparent border-0 text-center font-semibold hover:underline text-red-500">
            Logout
          </Button>
        </div>
      </div>
  ...
  return (
    <div className="w-full h-full flex flex-col bg-customWhite space-y-5">
      {tab === "folders" ? renderFolders() : renderSettings()}
